...
import { FieldGroup, GridField, MiniCard } from "../Inventory/Inventory";
import {
  Autocomplete,
  Box,
  Button,
...
const Admin = () => {
  ...
  const sections = [
    ...
    },
    {
      ...
      label: "Auditoría",
      icon: <HistoryIcon />,
      condition: authenticatedUser.is_superuser,
    },
    {
      id: "register",
  ...
  //Preguntar como se deben mostrar las alertas de consignación
  ...
  const downloadAuditLog = async () => {
    try {
      const response = await api.get(`/users/admin/export-audit-logs/`, {
        responseType: "blob",
      });
  ...
  const downloadManual = async (type) => {
    try {
      let manualURL =
        type === "admin"
          ? "/media/manuals/ManualAdministradorFlesystem.pdf"
          : "/media/manuals/ManualOperadorFlesystem.pdf";
  ...
  const downloadLowStockProducts = async () => {
    try {
      ...
      link.href = url;
      const today = moment();
      const download_name = `productos_stock_bajo_${today.format(
        "YYYY-MM-DD"
      )}.xlsx`;
      ...
      <MiniCard className={"max-w-full flex"}>
        ...
        <Box sx={{ marginBottom: 5 }}>
          ...
          <p>
            ...
            {authenticatedUser.is_superuser ? "administrador" : "operador"}
          </p>
        </Box>
        <Box
          sx={{
        ...
        {activeSection === "consignments" && (
          <Box>
            <ConsignmentManagerDialog />
          </Box>
        )}

        {activeSection == "creditsAlerts" && (
          <Box sx={{ marginTop: 5 }}>
            <Typography variant="h6">Alertas de Crédito</Typography>
            <CreditAlertsTable />
          </Box>
        )}
        {activeSection == "stock" && (
          ...
          >
            ...
            {authenticatedUser.kind_of_person == "operator" && (
              ...
              >
                <Typography variant="h6">Alertas de Crédito</Typography>
                <CreditAlertsTable />
                <Typography variant="h6">Consignamiento</Typography>
        ...
        {authenticatedUser.is_superuser && activeSection == "audit" && (
          <>
            ...
            <Box sx={{ display: "flex", gap: 5, marginTop: 5 }}></Box>
          </>
        )}
...
const AuditTable = ({ auditLogs }) => {
  ...
  const filteredAuditLogs = auditLogs.filter((log) => {
    const matchesEmail = log.user.email
      .toLowerCase()
    ...
    const matchesDate =
      (!startDate || logDate.isSameOrAfter(moment(startDate))) &&
...
const CreditAlertsTable = () => {
  const [alerts, setAlerts] = useState([]);
  const [alertsToShow, setAlertsToShow] = useState([]);
  const [filter, setFilter] = useState("all"); // "all", "paid", "unpaid"
  const { getCreditAlerts } = usePurchaseContext();

  useEffect(() => {
    const fetchAlerts = async () => {
      const data = await getCreditAlerts();
      setAlerts(data);
      setAlertsToShow(data);
    };
    fetchAlerts();
  }, []);
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              if (e.target.value === "paid") {
                setAlertsToShow(alerts.filter((alert) => alert.credit_paid));
            ...
            {alertsToShow?.map((alert) => (
              <TableRow key={alert.id}>
                ...
                <TableCell>
                  ...
                  >
                    {alert.credit_paid ? "Pagado" : "Pendiente"}
                  </Box>
                </TableCell>
                <TableCell>
                  <Button
                    ...
                    onClick={() => {
                      api
                        ...
                        .then(() => {
                          toast.success(
                            `Orden #${alert.id} marcada como pagada`
                          );
                          setAlertsToShow(
                            alerts.map((a) => {
                              if (a.id === alert.id) {
                                return { ...a, credit_paid: true };
                              }
                              return a;
                            })
                          );
                        ...
                        .catch((error) => {
                          toast.error(
                            `Error al marcar la orden: ${error.message}`
                          );
                        });
                    }}
                  >
